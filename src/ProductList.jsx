...
function ProductList() {
  ...
  const [showCart, setShowCart] = useState(false); // State to control the visibility of the products
  ...
  const handleAddToCart = (product) => {
    dispatch(addItem(product)); // handleAddToCart() function dispatches the plant details to the addItem() reducer function in CartSlice.jsx
    setAddedToCart((prevState) => ({
      ...prevState,
      [product.name]: true, // the product's name is used as a key to mark it as true
    }));
  };
  ...
  useEffect(() => {
    ...
    cartItems.forEach((item) => {
      updatedCartStatus[item.name] = true; // Mark each item in the cart as true
    });
    setAddedToCart(updatedCartStatus); // Update the state with the new cart status
  }, [cartItems]);
  ...
  return (
    <div>
          ...
          <div>
            <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
              <h1 className="cart">
                ...
                {cartItemsCount > 0 && (
                  <span className="cart-count">{cartItemsCount}</span> // Add this line
                )}
              </h1>
            </a>
          </div>
      ...
      {/* If showCart is false, show div with the class "product-grid".
      If showCart is true, display a CartItem component with a prop onContinueShopping*/}
      {!showCart ? (
